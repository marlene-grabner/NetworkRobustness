...
def standardize_and_verify(input_dir: str, output_dir: str):
    ...
    for file_path in files:
        ...
        if nodes_match and edges_match:
            print(
                f"  [SUCCESS] Verified! {len(new_nodes)} nodes and {len(new_edges)} edges preserved perfectly."
            )
        else:
            print(
                f"  [CRITICAL ERROR] Network structure changed during conversion for {filename}!"
            )
...
# Run the pipeline
# ==========================================
input_folder = "./data/baseline_networks/old_tsv_networks/"
output_folder = "./data/baseline_networks/"

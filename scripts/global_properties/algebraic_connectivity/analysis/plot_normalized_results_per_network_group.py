...
# --- Plotting Function ---
# ===========================================================================
# Same layout as plot_aggregated_results_per_network_group.py, but drawing the
# log2(Fiedler / baseline) ratio instead of the raw connectivity value, so an
# increase and a decrease of the same magnitude are symmetric around 0.

def plot_robustness_results(df, networks_to_plot, save_fig=None, view='all'):
    """
    Plots the log2 Fiedler-value ratio vs. noise level for a specific list of networks.
    """
    ...
    for i, net_id in enumerate(networks_to_plot):
        ...
        # Grab the baseline row once for this network (log2 ratio is 0 by definition)
        ...
        for p_type, style in target_styles.items():
            ...
            # Inject the baseline point so the line anchors at 100%
            if not baseline_row.empty:
                base_point = baseline_row.copy()
                base_point['noise_level'] = 100.0
            ...
            y = df_pert['avg_log2_ratio_fiedler']
            std = df_pert['std_log2_ratio_fiedler']
        ...
        # Baseline is always 0 in log2-ratio space
        ax.axhline(0.0, color='gray', linestyle=':', alpha=0.6, linewidth=1.5)
        x_min = ax.get_xlim()[0]
        y_range = y_max - y_min
        y_offset = y_range * 0.03
        ax.text(x_min, 0.0 + y_offset, ' Baseline (ratio = 1)',
        ...
        ax.set_ylabel(r"$\log_2$(Fiedler value / baseline)", fontsize=14, labelpad=10)
...
df = pd.read_csv("outputs/global_properties/algebraic_connectivity/aggregated_mean_fiedler_values_perturbed_plus_baseline_log2_normalized.csv")

# ============================================================================
# --- Plotting ---
# ===========================================================================
output_folder_figures = "outputs/global_properties/algebraic_connectivity/figures/normalized"

plot_robustness_results(df, ["ppi", "ppi_er", "ppi_conf", "ppi_sbm"], save_fig=f"{output_folder_figures}/all_ppis_all_perturbations_fiedler_value_log2norm.pdf")
plot_robustness_results(df, ["astro", "astro_er", "astro_conf", "astro_sbm"], save_fig=f"{output_folder_figures}/all_astros_all_perturbations_fiedler_value_log2norm.pdf")
plot_robustness_results(df, ["power", "power_er", "power_conf", "power_sbm"], save_fig=f"{output_folder_figures}/all_powers_all_perturbations_fiedler_value_log2norm.pdf")
plot_robustness_results(df, ["wiki", "wiki_er", "wiki_conf", "wiki_sbm"], save_fig=f"{output_folder_figures}/all_wikis_all_perturbations_fiedler_value_log2norm.pdf")

plot_robustness_results(df, ["astro", "ppi", "power", "wiki"], save_fig=f"{output_folder_figures}/all_base_networks_all_perturbations_fiedler_value_log2norm.pdf")

plot_robustness_results(df, ["ppi", "ppi_er", "ppi_conf", "ppi_sbm"], save_fig=f"{output_folder_figures}/all_ppis_removals_fiedler_value_log2norm.pdf", view='removal')
plot_robustness_results(df, ["astro", "astro_er", "astro_conf", "astro_sbm"], save_fig=f"{output_folder_figures}/all_astros_removals_fiedler_value_log2norm.pdf", view='removal')
plot_robustness_results(df, ["power", "power_er", "power_conf", "power_sbm"], save_fig=f"{output_folder_figures}/all_powers_removals_fiedler_value_log2norm.pdf", view='removal')
plot_robustness_results(df, ["wiki", "wiki_er", "wiki_conf", "wiki_sbm"], save_fig=f"{output_folder_figures}/all_wikis_removals_fiedler_value_log2norm.pdf", view='removal')

plot_robustness_results(df, ["ppi", "ppi_er", "ppi_conf", "ppi_sbm"], save_fig=f"{output_folder_figures}/all_ppis_additions_fiedler_value_log2norm.pdf", view='addition')
plot_robustness_results(df, ["astro", "astro_er", "astro_conf", "astro_sbm"], save_fig=f"{output_folder_figures}/all_astros_additions_fiedler_value_log2norm.pdf", view='addition')
plot_robustness_results(df, ["power", "power_er", "power_conf", "power_sbm"], save_fig=f"{output_folder_figures}/all_powers_additions_fiedler_value_log2norm.pdf", view='addition')
plot_robustness_results(df, ["wiki", "wiki_er", "wiki_conf", "wiki_sbm"], save_fig=f"{output_folder_figures}/all_wikis_additions_fiedler_value_log2norm.pdf", view='addition')
